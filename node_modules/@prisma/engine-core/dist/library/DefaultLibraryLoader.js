...
var import_handleEngineLoadingErrors = require("../common/errors/utils/handleEngineLoadingErrors");
...
function getLibraryCache() {
  ...
  if (globalWithCache[libraryCacheSymbol] === void 0) {
    globalWithCache[libraryCacheSymbol] = {};
  }
  return globalWithCache[libraryCacheSymbol];
}
__name(getLibraryCache, "getLibraryCache");
function load(libraryPath) {
  const cache = getLibraryCache();
  if (cache[libraryPath] !== void 0) {
    return cache[libraryPath];
  }
  const fullLibraryPath = import_path.default.toNamespacedPath(libraryPath);
  const libraryModule = { exports: {} };
  let flags = 0;
  if (process.platform !== "win32") {
    flags = import_os.default.constants.dlopen.RTLD_LAZY | import_os.default.constants.dlopen.RTLD_DEEPBIND;
  }
...
class DefaultLibraryLoader {
  ...
  async loadLibrary() {
    ...
    try {
      const enginePath2 = this.libQueryEnginePath;
      return (0, import_tracing.runInChildSpan)(
        { name: "loadLibrary", enabled: this.config.tracingConfig.enabled, internal: true },
        () => load(enginePath2)
      );
    } catch (e) {
      ...
      throw new import_PrismaClientInitializationError.PrismaClientInitializationError(errorMessage, this.config.clientVersion);
    }
  }
